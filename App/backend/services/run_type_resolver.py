from __future__ import annotations

from dataclasses import dataclass
from typing import Any
from uuid import UUID

from sqlalchemy.orm import Session

from ..models.db_models import RunModel, SubAgentDefinitionModel, Thread
from .settings_service import TaskConfig, settings_service
from .tool_schemas import ToolSchemaDef, get_dynamic_call_tools, get_tools_for_set


@dataclass
class HandlerDefaultOptions:
    create_new_version: bool
    user_request: str


@dataclass
class ResolvedRunConfig:
    llm_mode: str
    task_type: str
    provider: str
    provider_config: dict[str, Any]
    model: str
    temperature: float
    max_output_tokens: int | None
    thinking_mode: str
    thinking_config: dict[str, Any] | None
    thinking_format: str | None
    request_format: str | None
    tokenizer_override: str | None
    retry_config: dict[str, Any] | None
    tools: list[ToolSchemaDef]
    allowed_tool_names: list[str]
    context_window_tokens: int
    memory_enabled: bool
    history_strategy: str
    handler_default_options: HandlerDefaultOptions
    preset_id: UUID | None


@dataclass(frozen=True)
class ExecutionProfile:
    key: str
    memory_enabled: bool
    history_strategy: str
    handler_defaults: HandlerDefaultOptions


class RunTypeResolver:
    _PROFILES: dict[str, ExecutionProfile] = {
        "agent": ExecutionProfile(
            key="agent",
            memory_enabled=True,
            history_strategy="agent_root",
            handler_defaults=HandlerDefaultOptions(create_new_version=True, user_request="AI Edit"),
        ),
        "subAgent": ExecutionProfile(
            key="subAgent",
            memory_enabled=True,
            history_strategy="sub_agent",
            handler_defaults=HandlerDefaultOptions(create_new_version=True, user_request="Sub Agent"),
        ),
        "journey": ExecutionProfile(
            key="journey",
            memory_enabled=False,
            history_strategy="journey",
            handler_defaults=HandlerDefaultOptions(create_new_version=False, user_request="Journey"),
        ),
    }

    def _task_config_to_fields(
        self,
        task_cfg: TaskConfig,
    ) -> tuple[str, float, int | None, str, dict[str, Any] | None, str | None, str | None, str | None]:
        advanced = task_cfg.advanced if isinstance(task_cfg.advanced, dict) else {}
        thinking_mode = str(advanced.get("thinking_mode") or "off")
        thinking_cfg = advanced.get("thinking_config") if isinstance(advanced.get("thinking_config"), dict) else None
        thinking_format = advanced.get("thinking_format") if isinstance(advanced.get("thinking_format"), str) else None
        request_format = advanced.get("request_format") if isinstance(advanced.get("request_format"), str) else None
        tokenizer_override_raw = advanced.get("tokenizer_override")
        tokenizer_override = (
            str(tokenizer_override_raw)
            if isinstance(tokenizer_override_raw, str) and tokenizer_override_raw in {"openai", "claude", "gemini"}
            else None
        )
        return (
            task_cfg.model,
            task_cfg.temperature,
            task_cfg.max_output_tokens,
            thinking_mode,
            thinking_cfg,
            thinking_format,
            request_format,
            tokenizer_override,
        )

    def _resolve_agent_mode_and_tools(
        self,
        db: Session,
        *,
        run: RunModel,
        user_id: UUID,
        preset_id: UUID | None,
        rag_search_enabled: bool,
    ) -> tuple[str, str, list[ToolSchemaDef], TaskConfig]:
        llm_mode = f"agent:{run.run_mode}"
        set_name = "agent_plan_mode" if run.run_mode == "planMode" else "agent_agent_mode"
        dynamic = []
        if preset_id is not None and run.run_mode == "agentMode":
            dynamic = get_dynamic_call_tools(db, user_id=user_id, preset_id=preset_id)
        tools = get_tools_for_set(
            set_name,
            rag_search_enabled=rag_search_enabled,
            dynamic_call_tools=dynamic,
        )
        task_cfg = settings_service.get_task_config(db, user_id, "agent")
        return llm_mode, "agent", tools, task_cfg

    def _resolve_sub_agent_mode_and_tools(
        self,
        db: Session,
        *,
        run: RunModel,
        thread: Thread,
        user_id: UUID,
        preset_id: UUID | None,
        rag_search_enabled: bool,
    ) -> tuple[str, str, list[ToolSchemaDef], TaskConfig, float, int | None, str, dict[str, Any] | None, str | None, str | None, str | None, int]:
        owner_id = thread.owner_id
        if owner_id is None:
            raise ValueError("subAgent thread is missing owner_id")
        if preset_id is None:
            raise ValueError("No active preset for subAgent run")

        sub_agent = (
            db.query(SubAgentDefinitionModel)
            .filter(
                SubAgentDefinitionModel.user_id == user_id,
                SubAgentDefinitionModel.preset_id == preset_id,
                SubAgentDefinitionModel.id == owner_id,
            )
            .first()
        )
        if sub_agent is None:
            raise ValueError("Sub agent definition not found")

        llm_mode = f"subAgent:{sub_agent.agent_name}"

        allowed_static = sub_agent.allowed_tool_names if isinstance(sub_agent.allowed_tool_names, list) else []
        base_tools = [
            tool
            for tool in get_tools_for_set("agent_agent_mode", rag_search_enabled=rag_search_enabled)
            if tool.name in set(allowed_static)
        ]

        dynamic: list[ToolSchemaDef] = []
        allowed_child_ids = set(str(x) for x in (sub_agent.allowed_sub_agent_ids or []))
        if allowed_child_ids:
            defs = (
                db.query(SubAgentDefinitionModel)
                .filter(
                    SubAgentDefinitionModel.user_id == user_id,
                    SubAgentDefinitionModel.preset_id == preset_id,
                    SubAgentDefinitionModel.enabled == True,
                )
                .all()
            )
            for d in defs:
                if str(d.id) not in allowed_child_ids:
                    continue
                dynamic.append(
                    ToolSchemaDef(
                        name=f"call_{d.agent_name}",
                        description=f'Call Sub Agent "{d.display_name}".',
                        parameters={
                            "type": "object",
                            "additionalProperties": False,
                            "properties": {"input": {"type": "string"}},
                            "required": ["input"],
                        },
                    )
                )

        tools = [*base_tools, *dynamic]

        seen: set[str] = set()
        deduped: list[ToolSchemaDef] = []
        for t in tools:
            if t.name in seen:
                continue
            seen.add(t.name)
            deduped.append(t)
        tools = deduped

        if bool(sub_agent.use_custom_llm_config) and isinstance(sub_agent.llm_config_override, dict):
            override = sub_agent.llm_config_override
            fallback_cfg = settings_service.get_task_config(db, user_id, "subAgent")
            provider = str(override.get("provider") or fallback_cfg.provider)
            model = str(override.get("model") or fallback_cfg.model)
            temperature = float(override.get("temperature", fallback_cfg.temperature))
            max_tokens = override.get("max_output_tokens")
            advanced = override.get("advanced") if isinstance(override.get("advanced"), dict) else {}
            thinking_mode = str(advanced.get("thinking_mode") or "off")
            thinking_cfg = advanced.get("thinking_config") if isinstance(advanced.get("thinking_config"), dict) else None
            thinking_format = advanced.get("thinking_format") if isinstance(advanced.get("thinking_format"), str) else None
            request_format = advanced.get("request_format") if isinstance(advanced.get("request_format"), str) else None
            tokenizer_override_raw = advanced.get("tokenizer_override")
            tokenizer_override = (
                str(tokenizer_override_raw)
                if isinstance(tokenizer_override_raw, str) and tokenizer_override_raw in {"openai", "claude", "gemini"}
                else None
            )
            ctx_window = int(override.get("context_window_tokens") or 32000)
            task_cfg = settings_service.get_task_config(db, user_id, "subAgent")
            task_cfg.provider = provider
            task_cfg.model = model
        else:
            task_cfg = settings_service.get_task_config(db, user_id, "subAgent")
            model, temperature, max_tokens, thinking_mode, thinking_cfg, thinking_format, request_format, tokenizer_override = self._task_config_to_fields(task_cfg)
            ctx_window = task_cfg.context_window_tokens or 32000

        return (
            llm_mode,
            "subAgent",
            tools,
            task_cfg,
            temperature,
            max_tokens,
            thinking_mode,
            thinking_cfg,
            thinking_format,
            request_format,
            tokenizer_override,
            ctx_window,
        )

    def _resolve_journey_mode_and_tools(
        self,
        db: Session,
        *,
        run: RunModel,
        thread: Thread,
        user_id: UUID,
        rag_search_enabled: bool,
    ) -> tuple[str, str, list[ToolSchemaDef], TaskConfig, HandlerDefaultOptions]:
        journey_kind = thread.journey_kind
        if journey_kind == "translation":
            task_type = "translation"
            llm_mode = "journey:translation"
            tools = get_tools_for_set("translateObjects", rag_search_enabled=rag_search_enabled)
            defaults = HandlerDefaultOptions(create_new_version=False, user_request="AI Translation")
        elif journey_kind == "imagePrompt":
            task_type = "imagePrompt"
            llm_mode = "journey:imagePrompt"
            tools = []
            defaults = HandlerDefaultOptions(create_new_version=False, user_request="Image Prompt")
        else:
            task_type = "editAssistant"
            mode = run.input_payload.get("mode") if isinstance(run.input_payload, dict) else None
            set_name = "manuscript" if mode == "manuscript" else "storyObject"
            llm_mode = f"journey:aiEdit:{set_name}"
            tools = get_tools_for_set(set_name, rag_search_enabled=rag_search_enabled)
            defaults = HandlerDefaultOptions(create_new_version=True, user_request="AI Edit")

        task_cfg = settings_service.get_task_config(db, user_id, task_type)
        return llm_mode, task_type, tools, task_cfg, defaults

    async def resolve(self, db: Session, run: RunModel, user_id: UUID) -> ResolvedRunConfig:
        rag_settings = settings_service.get_rag_settings(db, user_id)
        preset_id = settings_service.get_active_preset_id(db, user_id)
        retry_config = settings_service.get_retry_config(db, user_id)

        thread = run.thread
        thread_type = thread.thread_type
        profile = self._PROFILES.get(thread_type)
        if profile is None:
            raise ValueError(f"Unknown thread type: {thread_type}")

        if profile.key == "agent":
            llm_mode, task_type, tools, task_cfg = self._resolve_agent_mode_and_tools(
                db,
                run=run,
                user_id=user_id,
                preset_id=preset_id,
                rag_search_enabled=rag_settings.enabled,
            )
            model, temp, max_tokens, thinking_mode, thinking_cfg, thinking_format, request_format, tokenizer_override = self._task_config_to_fields(task_cfg)
            ctx_window = task_cfg.context_window_tokens or 32000
            handler_defaults = profile.handler_defaults
        elif profile.key == "subAgent":
            (
                llm_mode,
                task_type,
                tools,
                task_cfg,
                temp,
                max_tokens,
                thinking_mode,
                thinking_cfg,
                thinking_format,
                request_format,
                tokenizer_override,
                ctx_window,
            ) = self._resolve_sub_agent_mode_and_tools(
                db,
                run=run,
                thread=thread,
                user_id=user_id,
                preset_id=preset_id,
                rag_search_enabled=rag_settings.enabled,
            )
            model = task_cfg.model
            handler_defaults = profile.handler_defaults
        else:
            llm_mode, task_type, tools, task_cfg, journey_defaults = self._resolve_journey_mode_and_tools(
                db,
                run=run,
                thread=thread,
                user_id=user_id,
                rag_search_enabled=rag_settings.enabled,
            )
            model, temp, max_tokens, thinking_mode, thinking_cfg, thinking_format, request_format, tokenizer_override = self._task_config_to_fields(task_cfg)
            ctx_window = task_cfg.context_window_tokens or 32000
            handler_defaults = journey_defaults

        return ResolvedRunConfig(
            llm_mode=llm_mode,
            task_type=task_type,
            provider=task_cfg.provider,
            provider_config={},
            model=model,
            temperature=temp,
            max_output_tokens=max_tokens,
            thinking_mode=thinking_mode,
            thinking_config=thinking_cfg,
            thinking_format=thinking_format,
            request_format=request_format,
            tokenizer_override=tokenizer_override,
            retry_config=retry_config,
            tools=tools,
            allowed_tool_names=[t.name for t in tools],
            context_window_tokens=ctx_window,
            memory_enabled=profile.memory_enabled,
            history_strategy=profile.history_strategy,
            handler_default_options=handler_defaults,
            preset_id=preset_id,
        )


run_type_resolver = RunTypeResolver()
