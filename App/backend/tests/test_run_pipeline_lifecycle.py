...
def test_cancel_run_allows_error_and_propagates_parent_completion(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    thread = Thread(
        id=uuid4(),
...
@pytest.mark.parametrize("run_status", ["paused", "error"])
def test_cancel_run_for_delete_cleans_up_paused_and_error_runs(run_status: str) -> None:
    thread = Thread(
        id=uuid4(),
        project_id=uuid4(),
        user_id=uuid4(),
    ...
    asyncio.run(
        stack.lifecycle.cancel_run_for_delete(
            thread_id=thread.id,
            user_id=thread.user_id,
        )
    )
