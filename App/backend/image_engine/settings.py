...
from ..provider_engine.contracts import MISSING, ObjectSpec
...
def _list_image_providers():
    providers = [provider for provider in list_providers() if provider.image is not None]
    providers.sort(
        key=lambda spec: (
...
def validate_image_gen_config(raw: Any) -> dict[str, Any]:
    ...
    provider_spec = require_provider(config.provider)
    ...
    for provider in _list_image_providers():
        ...
        cleaned = sanitize_provider_settings(provider.id, stored or {})
