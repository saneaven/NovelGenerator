...
export const ActionBadge: React.FC<ActionBadgeProps> = ({ action, type, className = '' }) => {
  ...
  return (
    <div className={`function-call-action-badges ${className}`}>
      <span className={`function-call-action-badge function-call-action-badge--${action || 'default'}`}>
        {actionLabel}
      </span>
      {typeLabel && <span className="function-call-type-label">{typeLabel}</span>}
    </div>
  );
};
